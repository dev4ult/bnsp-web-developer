...
<div class="p-8">
    ...
    <form action="{{ route('student.store') }}" method="POST" class="shadow p-8 min-w-52 rounded-lg">
        @csrf
        @method('POST')

        <h1 class="text-3xl font-semibold">Registrasi Akun Mahasiswa</h1>
        ...
        <div class="grid grid-flow-row grid-cols-2 gap-x-5 gap-y-3 mt-5">
            ...
            <div class="space-y-1.5">
                <label for="password" class="font-medium text-sm">Password (default: <span class="underline text-red-500">mhsw1234</span>) *</label>
                <input type="password" id="password" name="password" class="input input-bordered w-full" required value="mhsw1234" />
            </div>

        </div>

        <button type="submit" class="btn btn-block btn-primary mt-6">Registrasi / Tambah Akun</button>
    </form>
</div>
@endsection